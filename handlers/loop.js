...
//handler - forEach of an iterable set - Not stopping even after a failure in single item
        ...
        function (resolve, reject) {
            ...
            try {
                for (let i = 0; i < iterableSet.length; i++) {
                    ...
                    //if a onEach is rejected , reject the whole function
                    if (isRejected) {
                        //running a simple function If there is an error
                        onEachError(position++, rejectedError);
                        isRejected = false;
                        wait = false;
                    } else {
                        wait = false;
